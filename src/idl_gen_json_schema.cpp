        ...
        class JsonSchemaGenerator : public BaseGenerator {
            ...
            public:
                ...
                bool generate() {
                    code_.Clear();
                    cpp_gen_.Clear();
                    cpp_gen_ += "#ifndef _GEN_HPP_\
                                 #define _GEN_HPP_\
                                 template<typename stringSetterFuncType, typename stringGetterFuncType, typename setterFuncType, typename getterFuncType>\
                                 class UpdateManager {\
                                     public:\
                                            ";
                    ...
                    for (auto s = parser_.structs_.vec.cbegin();
                        ...
                        for (auto prop = properties.cbegin(); prop != properties.cend(); ++prop) {
                            ...
                            std::string attr = "";
                            std::string d ="";
                            if (property->attributes.dict.size()) {
                            for (auto v: property->attributes.dict) {
                                attr += "\"" + v.first +  "\" : " +  v.second->constant + " ,";
                                if (v.first == "param") {
                                    cpp_gen_+= "virtual void ParamBuilder<\""+property->name+"\">(std::string, setterFuncType , getterFuncType = nullptr, stringSetterFuncType = nullptr, stringGetterFuncType = nullptr ) = 0;\n";
                                }
                    ...
                    const std::string file_path2 = GeneratedFileName(path_, "basket.gen.cpp");

                    const std::string final_code = code_.ToString();
                    const std::string cppcode = cpp_gen_.ToString();
                    SaveFile(file_path2.c_str(), cppcode, false);
